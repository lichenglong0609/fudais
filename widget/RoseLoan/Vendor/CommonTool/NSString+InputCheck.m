...
{
    if([@"➋➌➍➎➏➐➑➒" rangeOfString:self].location !=NSNotFound)
    {
        return YES;
    }
...
- (BOOL) validateCombination{//组合类型:字母,数字,符号 - 任意两种以上组合
    
    NSString *nicknameRegex = @"^(?![A-Z]+$)(?![a-z]+$)(?!\\d+$)(?![\\W_]+$)\\S+$";
...
- (BOOL)haveSpecialChar {
    ...
    NSLog(@"%d",[predicate evaluateWithObject:self]);
...
- (BOOL)valiteCreditCard{
    NSString *cardRegex = @"^\\d{n}$";
    NSPredicate *cardPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",cardRegex];
    
    return [cardPredicate evaluateWithObject:self];
}

- (BOOL)valiteLimitedDate{
    NSString *cardRegex = @"^(0[1-9]|1[0-2])/(\\d{2})$";
    NSPredicate *cardPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",cardRegex];
...
- (NSString *)returnCityId:(id)cityId{
    ...
    if ([cityId isKindOfClass:[NSString class]]) {
        ...
        return [NSString stringWithFormat:@"%@",cityId];
    }
